...
func _process(_delta: float) -> void:
	# 1. Verifica regeneração primeiro (funciona mesmo se estiver quebrado)
	if Input.is_key_pressed(KEY_SPACE):
		regenerate()
		return
	
	# 2. Se já estiver quebrado, não processa movimento/quebra novamente
	if broken:
		return
...
func activate_physics(direction: Vector2, force_level: int) -> void:
	...
	for piece in pieces.get_children():
		if piece is RigidBody2D:
			...
			# --- O AJUSTE ESTÁ AQUI ---
			# Desliga a colisão com a Layer 1 (outras peças)
			piece.set_collision_mask_value(1, false)
			
			# Garante que a colisão com a Layer 2 (chão) esteja LIGADA
